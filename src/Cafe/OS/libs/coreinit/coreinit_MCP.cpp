...
// Categorize titleId-high (full 32-bit "high" portion) into the slot kinds
// drmapp / CheckTitleEnability treats as "optional, absence is benign".
// Update (0x0005000e) and DLC/AOC (0x0005000c) are the two relevant ones.
static bool _MCP_IsOptionalSlotTitle(uint64 titleId)
{
	uint32 titleIdHigh = (uint32)(titleId >> 32);
	return titleIdHigh == 0x0005000eu || titleIdHigh == 0x0005000cu;
}
...
void coreinitExport_MCP_GetTitleInfoByTitleAndDevice(PPCInterpreter_t* hCPU)
{
	...
	if (mcpRequest->titleListRequest.titleCount == 0)
	{
		...
		// See _MCP_IsOptionalSlotTitle: short-circuit update/DLC misses so
		// drmapp's CheckTitleEnability doesn't bail out with 199-9999 on
		// base-only retail game launches.
		if (_MCP_IsOptionalSlotTitle(titleId))
		{
			osLib_returnFromFunction(hCPU, NN_RESULT_SUCCESS);
			return;
